Game.Builder = function(width, height, depth) {
	...
	// Instantiate the arrays to be milti-dimension
	for (var z = 0; z < depth; z++) {
		// Create a new cave at each level
		this._tiles[z] = this._generatedLevel();
...
Game.Builder.prototype._setupRegions = function(z) {
	var region = 1;
	var tilesFilled;
	// Iterate through all tiles searching for a tile that can be used as starting point for flood fill
	for (var x = 0; x < this._width; x++) {
		for (var y = 0; y < this._height; y++) {
...
// This tries to connect two regions by calculating where they overlap and adding stairs
...
// This tries to connect all regions for each depth level, starting from top most depth level.
Game.Builder.prototype._connectAllRegions = function() {
	for (var z = 0; z < this._depth - 1; z++) {
		// Iterate through each tile, and if we haven't tried to connect the region of that tile on both depth levels
		// then we try. We store the connected properties as strings for quick lookups.
		var connected = {};
		var key;
		for (var x = 0; x < this._width; x++) {
			for (var y = 0; y < this._height; y++) {
				...
				if (this._tiles[z][x][y] == Game.Tile.floorTile && 
				    this._tiles[z+1][x][y] == Game.Tile.floorTile && !connected[key]) {
					// Since both tiles are floors and we haven't already connected, try now
